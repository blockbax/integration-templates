...
const { pathExists, readFile } = require("../utils/utils");
...
function newTemplateConfigPath(groupId, templateId, options = {}) {
    const groupPath = newGroupPath(groupId, options);
    return path.join(groupPath, templateId, constants.templateConfigFileName);
}
...
function getTests(groupId, templateId, options = {}) {
    const testsPath = newTemplateTestSuitePath(groupId, templateId, options);
